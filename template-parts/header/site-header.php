...
<header class="negarin-header sticky top-0 z-40 bg-white border-b border-negarin-line" x-data="{ menuOpen: false, searchOpen: false }">
    <div class="container max-w-7xl mx-auto px-4 py-4 flex flex-row-reverse items-center justify-between">
        ...
        <div class="flex items-center gap-4 text-lg flex-row-reverse">
            <a href="<?php echo esc_url( function_exists( 'wc_get_account_endpoint_url' ) ? wc_get_account_endpoint_url( 'edit-account' ) : '#' ); ?>" aria-label="<?php esc_attr_e( 'حساب کاربری', 'negarin' ); ?>">
            ...
            <button @click="searchOpen = true" aria-label="<?php esc_attr_e( 'جستجو', 'negarin' ); ?>">
            ...
            <div class="relative group">
                <a href="<?php echo esc_url( function_exists( 'wc_get_cart_url' ) ? wc_get_cart_url() : '#' ); ?>" class="relative" aria-label="<?php esc_attr_e( 'سبد خرید', 'negarin' ); ?>">
                    ...
                    <?php get_template_part( 'template-parts/components/cart-drawer-count' ); ?>
                </a>
                <?php
                // Hover dropdown — desktop only (md:group-hover:block on the
                // component itself, via the `group` class on this wrapper).
                // On mobile the <a> above just navigates to the cart page as
                // it always did; hover doesn't apply there.

...
?>
<header class="negarin-header sticky top-0 z-40 bg-white border-b border-black/5" x-data="{ menuOpen: false, searchOpen: false }">
	<div class="max-w-7xl mx-auto px-4 py-4 flex flex-row-reverse items-center justify-between">
		...
		<div class="flex items-center gap-4 text-lg flex-row-reverse">
			...
			<a href="<?php echo esc_url( function_exists( 'wc_get_cart_url' ) ? wc_get_cart_url() : '#' ); ?>" class="relative" aria-label="<?php esc_attr_e( 'سبد خرید', 'negarin' ); ?>">
                ...
                <span class="negarin-cart-count absolute -top-2 -left-2 text-[10px] bg-negarin-ink text-white rounded-full w-4 h-4 flex items-center justify-center">

...
<div <?php wc_product_class( 'product-card group', $product ); ?>>
	...
	<a href="<?php echo esc_url( get_permalink() ); ?>" class="block relative overflow-hidden bg-negarin-cream aspect-[3/4]">
		...
		<?php if ( $main_id ) : ?>
			<?php negarin_image( (int) $main_id, 'negarin-product-card', 'absolute inset-0 w-full h-full object-cover transition-opacity duration-500' . ( $hover_id ? ' group-hover:opacity-0' : '' ) ); ?>
		<?php else : ?>
			<?php echo wc_placeholder_img( 'negarin-product-card', array( 'class' => 'absolute inset-0 w-full h-full object-cover' ) ); // phpcs:ignore ?>
		<?php endif; ?>

		<?php if ( $hover_id ) : ?>
			<?php negarin_image( (int) $hover_id, 'negarin-product-card', 'absolute inset-0 w-full h-full object-cover opacity-0 transition-opacity duration-500 group-hover:opacity-100' ); ?>
		<?php endif; ?>

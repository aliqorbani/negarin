...
class OffcanvasMenuWalker extends \Walker_Nav_Menu {
    ...
    public function start_lvl( &$output, $depth = 0, $args = null ) {
        ...
        $output .= '<div class="relative flex items-center justify-center px-4 py-5 border-b border-black/10">';
        $output .= '<button type="button" class="absolute left-4 text-xl leading-none" @click="panel = 0" aria-label="' . esc_attr__( 'بازگشت', 'negarin' ) . '">‹</button>';
        $output .= '<span class="font-serif tracking-[0.3em]">' . esc_html( get_bloginfo( 'name' ) ) . '</span>';
        $output .= '</div>';
        $output .= '<ul>';
    }

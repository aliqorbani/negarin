...
class CustomOrder {
    ...
    public function handle_add_to_cart( WP_REST_Request $request ) {
        ...
        // WooCommerce refuses to add a variable product to the cart
        // without a real variation_id (hard-coded in WC_Cart::add_to_cart(),
        // not filterable) — a custom order has no actual size, so it uses a
        // dedicated hidden variation created on first use. See
        // Services/ProductSizing.php::get_or_create_custom_order_variation().
        $variation_id         = 0;
        $variation_attributes = array();

        if ( $product instanceof \WC_Product_Variable ) {
            $variation_id = \Negarin\Services\ProductSizing::get_or_create_custom_order_variation( $product );

            if ( ! $variation_id ) {
                return new WP_Error( 'negarin_add_to_cart_failed', __( 'افزودن به سبد خرید با خطا مواجه شد.', 'negarin' ), array( 'status' => 400 ) );
            }

            $variation_attributes = array(
                'attribute_' . \Negarin\Services\ProductSizing::taxonomy() => \Negarin\Services\ProductSizing::CUSTOM_ORDER_TERM_SLUG,
            );
        }
